##############################################################################
# credits to: https://github.com/Keyaru-code/android-sdk-termux
##############################################################################
...
uninstall() {
    ...
    remove_existing_env "$BASHRC" "bash"
    
    if [ -f "$ZSHRC" ]; then
        remove_existing_env "$ZSHRC" "zsh"
    fi
    
    if [ -f "$FISH_CONFIG" ]; then
        remove_existing_env "$FISH_CONFIG" "fish"
    fi

    log_success "Android SDK completely removed"
}
